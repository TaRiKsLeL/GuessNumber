$(document).ready(function () {
  ...
  function createUsersTable() {
    ...
    return "<table class='users-table'>" + headerTable + innerTable + "</table>";
  }
  ...
  $("#input_num").click(function () {
    ...
    if (currentUserName !== inputName && users.some((user) => user.name === inputName)) {
      // Якщо ввели користувача, який раніше був введений, очистити дані про нього
      users.find((user) => user.name === inputName).attempts = 0;
    }

    currentUserName = inputName;

    if (users.some((user) => user.name === currentUserName)) {
      //Якщо міститься такий користувач в списку, якщо ні - створити
  ...
  function win() {
    ...
    $(".rating-text").html(`<p>Рейтинг</p>` + createUsersTable());
    $("#rez .hint").css("color", "#ffcd7a");
    $("#user_name").val("");
    $("#you_num").val("");
  }
});
